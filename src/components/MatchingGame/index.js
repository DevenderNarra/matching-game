...
class MatchingGame extends Component {
  ...
  handleThumbnailClick = image => {
    ...
    if (image.imageUrl === currentImage) {
  ...
  render() {
    const {score, time, currentImage, activeTab, gameOver} = this.state
        ...
        {!gameOver ? (
          <>
            ...
            <div className="current-image">
              <img src={currentImage} alt="match" />
            </div>
            <ul className="tabs">
              {tabsList.map(tab => (
                <li>
          ...
          <Scorecard score={score} onPlayAgain={this.handlePlayAgain} />
        )}
      </div>
    )
  }
